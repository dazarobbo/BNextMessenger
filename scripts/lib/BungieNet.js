 ...
 *
 * [public member] getAll( void ) : Promise(array of Cookie)
...
((__scope) => {
	...
	Object.defineProperties(BungieNet, {
		...
		base: {
			/**
			 * Base Bungie.net URI with scheme
			 * @return {URI}
			 */
			get: () => {
				return new URI({
			...
			value: () => {
				return new Promise((resolve) => {
					...
					BungieNet.currentUser.getLocale().then((loc) => {
						return resolve(loc); //TODO: superfluous?
					}, () => {
	...
	Object.defineProperties(BungieNet.cookies, {
		...
		getAll: {
			 ...
			 */
			value: () => {
				return BungieNet.cookies.provider
		...
		provider: {
			/**
			 * Interface to a cookie provider (see initial notes)
			 */
			value: null,
			writable: true
		}
	...
	Object.defineProperties(BungieNet.currentUser, {
		...
		getLocale: {
			 ...
			 * @return {Promise} resolves with string if successful, otherwise
			 * rejected with null
			 */
			value: () => {
				return new Promise((resolve, reject) => {
	...
	Object.defineProperties(BungieNet.Platform.prototype, {
		...
		/**
		 * getter/setter for requesting as a user
		 * @type {bool}
		 */
		userContext: {
			set: function(ok) {
		...
		/**
		 * getter/setter for network timeout
		 * @type {int}
		 */
		timeout: {
		...
		/**
		 * Services available to this instance
		 * @type {Object}
		 */
		services: {
			value: { },
	...
	/**
	 * Service constructor
	 * @param  {String} namespace service namespace according to bungie.net
	 * @param  {BungieNet.Platform} platform  platform instance
	...
	BungieNet.Platform.Service = function(namespace, platform, endpoints) {
		...
		//bind each endpoint to this service instance
						...
						this._getPlatform()
							...
							.then((respText) => {
								...
								try{
									...
									//if fail, assume corrupt/non-JSON response
								...
								//return a platform response type
				...
				return {
					...
					message: new BungieNet.Platform.Service("Message", platform, {
						...
						getConversationThreadV3: function(id, page, after, before) {
							...
							return this._serviceRequest(uri);
	...
	Object.defineProperties(BungieNet.User.prototype, {
		...
		getProfileLink: {
			/**
			 * Generates the most appropriate URI to a profile
			 * @return {URI} [description]
			 */
			value: () => {
				return new Promise((resolve) => {
	...
	Object.defineProperties(BungieNet.User, {
		...
		defaultAvatar: {
			/**
			 * Returns the most appropriate link to the default bungie.net avatar
			 * @return {URI} [description]
			 */
			get: () => {
